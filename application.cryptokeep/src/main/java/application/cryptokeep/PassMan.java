...
import application.cryptokeep.encryption.cryptor;
...
public class PassMan extends JFrame{
	...
	private static ArrayList <String> uname = new ArrayList<String>();
	...
	private static ArrayList<String> group11 = new ArrayList<String>();
	private static ArrayList<String> group22 = new ArrayList<String>();
	private static ArrayList<String> linkS = new ArrayList<String>();
	//The pin number and username
	
	private static String userIden;
	...
	public void setMenuBar1() {
		...
		export.addActionListener(new exportData2());
		import1.addActionListener(new importData());
	...
	public static boolean flag(){
		...
		File file = new File("pobj.ser");
		File def = new File("default.txt");
		
		if(file.exists() && def.exists()) {
			return true;
		}
			else{return false;}
	...
	//start a new file if data is not set , this is called IF first function is false
		...
		PrintWriter objectFile = new PrintWriter("pobj.ser");
		...
		userIden = JOptionPane.showInputDialog("Please create a username to proceed");
		String pinString = JOptionPane.showInputDialog("Please create PIN number Master key to proceed");
			...
			byte [] userHash = md.digest(userIden.getBytes());
			byte [] userPin = md.digest(pinString.getBytes());
			String finalUser = new String(userHash);
		...
		ADV_IO keyStore = new ADV_IO();
			...
			keyStore.fileStart(finalUser, "user.txt");
			...
			pObject = new passObject(finalUser, pin, usernames, passwords, uname, favorites, group11, group22, linkS);
			
			
			keyStore.storeObject(pObject, new File("pobj.ser"));
    ...
    public static void load_txt_elements() throws IOException, ClassNotFoundException{
    	...
    	*/
    	usernames.addAll(pObject.getUsernames());
    	...
    	passwords.addAll(pObject.getPasswords());
...
public static void auth() throws FileNotFoundException, NoSuchAlgorithmException{
	...
	public static void deleteEntry(String entry) throws IOException {
		
		int index;
		LinkedList <String> lyst = new LinkedList<String>();
		...
		for(int i = 0; i < lyst.size(); i++) {
			...
			if(lyst.get(i).equals(entry)) {
				...
				new ADV_IO().storeObject(pObject, new File("pobj.ser"));
    ...
    protected class ADD_AND_ENCRYPT implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent e) {
    		...
    		String uname1 = textUser.getText();
    		...
    		try {
				...
				byte[] encryptedUser = crypt.encrypt(uname1.getBytes(), GetProperKey(), "def.txt");
				...
				ADV_IO io = new ADV_IO();
				 ...
				 usernames.add(getuBytes);
				 passwords.add(getpBytes);
				 model.addElement(uname1);
				 uname.add(uname1);
...
*/
	...
	private class exportData2 implements ActionListener{
		
		public void actionPerformed(ActionEvent e) {
			
			ObjectMapper map = new ObjectMapper();
			
			passObject obj = new passObject(userIden, pin, usernames, passwords, uname, favorites, group11, group22, linkS);
			
			String user = System.getProperty("user.name");
			final String outPath = user+"-cryptokeep-data.json";
			
			File file = new File(outPath);
			
			try {
				map.writeValue(file, obj);
	...
	private class importData implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			ObjectMapper map = new ObjectMapper();
			passObject pobj = new passObject();
			
			try {
				pobj = map.readValue(file, passObject.class);
			...
			for(String x : pobj.getUname()) {
				model.addElement(x);
			}

			ArrayList<String> tempPass = new ArrayList<String>();
			ArrayList<String> tempUsernames = new ArrayList<String>();
			ArrayList<String> tempUname = new ArrayList<String>();
			ArrayList<String> tempFavorites = new ArrayList<String>();
			ArrayList<String> tempGrp1 = new ArrayList<String>();
			ArrayList<String> tempGrp2 = new ArrayList<String>();
			ArrayList<String> tempLnk = new ArrayList<String>();
			
			tempPass.addAll(pobj.getPasswords());
			tempPass.addAll(pObject.getPasswords());
			pObject.setPasswords(tempPass);
			
			tempUsernames.addAll(pobj.getUsernames());
			tempUsernames.addAll(pObject.getUsernames());
			pObject.setUsernames(tempUsernames);
			
			tempUname.addAll(pobj.getUname());
			tempUname.addAll(pObject.getUname());
			pObject.setUname(tempUname);
			
			tempFavorites.addAll(pobj.getFavorites());
			tempFavorites.addAll(pObject.getFavorites());
			pObject.setFavorites(tempFavorites);
			
			tempGrp1.addAll(pobj.getGroup11());
			tempGrp1.addAll(pObject.getGroup11());
			pObject.setGroup11(tempGrp1);
			
			tempGrp2.addAll(pobj.getGroup22());
			tempGrp2.addAll(pObject.getGroup22());
			pObject.setGroup22(tempGrp2);
			
			tempLnk.addAll(pobj.getLinkS());
			tempLnk.addAll(pObject.getLinkS());
			pObject.setLinkS(tempLnk);
